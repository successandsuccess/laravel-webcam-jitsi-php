...
<body>
    ...
    <script>
        ...
        var background;
        // var background_image_url = "http://app.guvavet.com/assets/images/animals/dog.jpg";
        var background_image_url = '<?php echo $imageUrl; ?>';
        var initwidth = ($(window).width() - 30) + "px";
        var initheight = ($(window).height() - 30) + "px";
        ...
        (function($) {
            $.fn.simplePaint = function(options) {
                var settings = $.extend({
                    ...
                    width: initwidth,
                    height: initheight,
                    // buttons: ["line", "triangle", "rect", "circle", "text", "eraser", "undo", "redo", "clear", "image", "save"],
                    buttons: ["pen", "line", "triangle", "rect", "circle", "text", "undo", "redo", "moveRight"],
                    canvasBgColor: "rgb(255, 255, 253)"
                }, options);
                ...
                var method = {
                    ...
                    setBgImage: function(imgData, x, y, w, h, type, imageOrgin) {
                        method.canvas.getContext('2d').clearRect(0, 0, w, h);
                        var img = new Image();
                        img.onload = function() {
                            var newrate = h / img.height;
                            w = newrate * img.width;
                            method.context.save();
                            method.context.drawImage(img, 0, 0, w, h);
                            method.context.restore();
                            var imgSize = { w: w, h: h };
                            var imgPos = { x: x, y: y };
                            method._currentStroke.color = ""; //method.penColor;
                    ...
                    init: function() {
                        ...
                        background.onload = function() {
                            var rate = background.width / background.height;
                            initwidth = rate * (initheight - 80);
                            method.context.drawImage(background, 0, 0, initwidth, initheight - 80);
                        }
                    ...
                    redraw: function(strokes) {
                        ...
                        // method.context.drawImage(background,0,0);

                        var rrrate = background.width / background.height;
                        initwidth = rrrate * (initheight - 80);
                    ...
                    saveBtn: function() {
                        ...
                        $(".simple-paint-save-btn").html(saveBtn).on("click", function() {
                            ...
                            //var data = document.getElementById("canvasDiv").toDataURL();
                            ...
                            */
                            ...
                            /*  EL DE LA SIGNATURE
                            var dataURL = document.getElementById("canvasDiv").toDataURL();
                            ...
                            */
                            ...
                            /* EL QUE HI HAVIA ORIGINAL. PER DESCARREGAR
                    ...
                    zoomIn: function() {
                        var zoomInBtn = '<button class="btn-zoom">zoom+</button>';
                        $("<span class='simple-paint-tooltip simple-paint-buttons simple-paint-zoomIn-btn' title='Zoom In as image' />").appendTo($(".simple-paint-top-tools"));
                        $(".simple-paint-zoomIn-btn").html(zoomInBtn).on("click", method.actZoomIn);
                    },
                    zoomOut: function() {
                        var zoomOutBtn = '<button class="btn-zoom">zoom-</button>';
                        $("<span class='simple-paint-tooltip simple-paint-buttons simple-paint-zoomOut-btn' title='Zoom In as image' />").appendTo($(".simple-paint-top-tools"));
                        $(".simple-paint-zoomOut-btn").html(zoomOutBtn).on("click", method.actZoomOut);
                    },
                    // moveWidthRight: function() {
                    //     var moveRightBtn = '<button class="btn-zoom">Right</button>';
                    //     $("<span class='simple-paint-tooltip simple-paint-buttons simple-paint-moveright-btn' title='Move image Right' />").appendTo($(".simple-paint-top-tools"));
                    //     $(".simple-paint-moveright-btn").html(moveRightBtn).on("click", method.movewidthright);
                    // },
                    moveWidthRight: function() {
                        var moveRightBtn = '<button class="btn-zoom">Clear</button>';
                        $("<span class='simple-paint-tooltip simple-paint-buttons simple-paint-moveright-btn' title='Clear Image' />").appendTo($(".simple-paint-top-tools"));
                        $(".simple-paint-moveright-btn").html(moveRightBtn).on("click", method.movewidthright);
                    },
                    moveWidthLeft: function() {
                        var moveLeftBtn = '<button class="btn-zoom">Left</button>';
                        $("<span class='simple-paint-tooltip simple-paint-buttons simple-paint-moveleft-btn' title='Move image Left' />").appendTo($(".simple-paint-top-tools"));
                        $(".simple-paint-moveleft-btn").html(moveLeftBtn).on("click", method.movewidthleft);
                    },
                    moveHightUp: function() {
                        var moveUpBtn = '<button class="btn-zoom">Up</button>';
                        $("<span class='simple-paint-tooltip simple-paint-buttons simple-paint-moveup-btn' title='Move image Up' />").appendTo($(".simple-paint-top-tools"));
                        $(".simple-paint-moveup-btn").html(moveUpBtn).on("click", method.movehightup);
                    },
                    moveHightDown: function() {
                        var moveDownBtn = '<button class="btn-zoom">Down</button>';
                        $("<span class='simple-paint-tooltip simple-paint-buttons simple-paint-movedown-btn' title='Move image Down' />").appendTo($(".simple-paint-top-tools"));
                        $(".simple-paint-movedown-btn").html(moveDownBtn).on("click", method.movehightdown);
                    }
                ...
                $.each(settings.buttons, function(i, button) {
                    switch (button) {
                        ...
                        case "zoomOut":
                            ...
                            break;
                        case "moveRight":
                            method.moveWidthRight();
                            break;
                        case "moveLeft":
                            method.moveWidthLeft();
                            break;
                        case "moveUp":
                            method.moveHightUp();
                            break;
                        case "moveDown":
                            method.moveHightDown();
                            break;
                    }
                })
                method.init();
