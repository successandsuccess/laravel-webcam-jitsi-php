...
class DropRxsInDxs extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::table('dxs', function (Blueprint $table) {
            if (Schema::hasColumn('dxs', 'rx_1')) {
                $table->dropForeign('dxs_rx_1_foreign');
                $table->dropColumn('rx_1');
                $table->dropForeign('dxs_rx_2_foreign');
                $table->dropColumn('rx_2');
                $table->dropForeign('dxs_rx_3_foreign');
                $table->dropColumn('rx_3');
                $table->dropForeign('dxs_rx_4_foreign');
                $table->dropColumn('rx_4');
                $table->dropForeign('dxs_rx_5_foreign');
